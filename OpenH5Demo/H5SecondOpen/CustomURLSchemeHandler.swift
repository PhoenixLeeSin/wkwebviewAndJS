...
extension CustomURLSchemeHandler {
    ...
    func requestRomote(fileName: String, urlSchemeTask: WKURLSchemeTask) {
        // 没有缓存,替换url，重新加载
        guard let urlString = urlSchemeTask.request.url?.absoluteString.replacingOccurrences(of: "customscheme", with: "http") else { return }
        print("开始重新发送网络请求")
        // 替换成https请求
...
extension CustomURLSchemeHandler: WKURLSchemeHandler {
    ...
    func webView(_ webView: WKWebView, start urlSchemeTask: WKURLSchemeTask) {
        ...
        if accept.count >= "text".count && accept.contains("text/html") {
            ...
            guard let originUrlString = urlSchemeTask.request.url?.absoluteString.replacingOccurrences(of: "customscheme", with: "http") else { return }
